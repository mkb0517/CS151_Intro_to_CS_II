...
// Version     : 1.11
// Copyright   : None!
// Description : Algorithms for sorting and removing without using <algorithm> from STL
//==========================================================================*/
...
public:
    ...
    void fixup() {
        ...
        for (unsigned int i = 0; i < rows.size(); i++){
            if (rows[i].amount==0){
                while(i<rows.size()){
                    rows[i].amount = rows[i+1].amount;
                    rows[i].item = rows[i+1].item;
                    i++;
                }
                rows.resize(rows.size()-1);
            }
        }
        // Sort all rows according from lowest to highest
        for (unsigned int i = 0; i < rows.size(); i++) {
    ...
    bool operator-=(Selection remove) {
        bool found = false;
        for(unsigned int i=0; i<rows.size(); i++){
            if (rows[i].item==remove.item){
                rows[i].amount -= remove.amount;
                found = true;
            }
            if (rows[i].amount < 0){
                return false;
...
class NegativeException : public exception {
    const char *message="  disqualified for removing too many from a row!";
public:
    const char *what() {
        return message;
    }
...
class InvalidException : public exception {
    const char *message="  was disqualified for trying to remove an item from a row that isn't there!";
public:
    const char *what() {
        return message;
    }
